...
photos_application_refresh_miners (PhotosApplication *self)
{
  ...
  if (photos_source_manager_has_provider_type (PHOTOS_SOURCE_MANAGER (priv->state->src_mngr), "google"))
    photos_application_refresh_miner_now (self, priv->picasaweb_miner);
}
...
photos_application_startup (GApplication *application)
{
  ...
  priv->picasaweb_miner = gom_miner_proxy_new_for_bus_sync (G_BUS_TYPE_SESSION,
                                                            G_DBUS_PROXY_FLAGS_NONE,
...
photos_application_dispose (GObject *object)
{
  ...
  g_clear_object (&priv->picasaweb_miner);
